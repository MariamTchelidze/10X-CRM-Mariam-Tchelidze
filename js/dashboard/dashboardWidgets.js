...
(function initDashboardWidgets() {
  ...
  widget.innerHTML = `
    <button class="workspace-widget__fab js-ai-toggle" type="button" aria-expanded="false" aria-controls="ai-assistant-panel">
      <img src="./assets/icons/chat.svg" data-theme-src-dark="./assets/icons/chat.svg" data-theme-src-light="./assets/icons/chat-light-theme.svg" alt="" />
      <span>AI</span>
    </button>
    <section class="workspace-widget__panel ai-assistant js-ai-panel" id="ai-assistant-panel" hidden>
      <header class="workspace-widget__header">
        <strong>CRM Assistant</strong>
        <button class="icon-btn js-ai-toggle" type="button" aria-label="Close assistant">
          <img src="./assets/icons/close.svg" data-theme-src-dark="./assets/icons/close.svg" data-theme-src-light="./assets/icons/close-light-theme.svg" alt="" />
        </button>
      </header>
      <div class="workspace-widget__messages js-ai-messages">
        <p class="workspace-widget__message">Ask me to draft a follow-up, summarize leads, or create a task.</p>
      </div>
      <form class="workspace-widget__form js-ai-form">
        <input class="input js-ai-input" type="text" placeholder="Try: create task for Alpha Group" />
  ...
  const aiPanel = widget.querySelector(".js-ai-panel");
  const toggleAi = () => {
    aiPanel.hidden = !aiPanel.hidden;
    widget.querySelector(".js-ai-toggle")?.setAttribute("aria-expanded", String(!aiPanel.hidden));
  };
  widget.querySelectorAll(".js-ai-toggle").forEach((button) => button.addEventListener("click", toggleAi));
  widget.querySelector(".js-ai-form")?.addEventListener("submit", (event) => {
    event.preventDefault();
    const input = widget.querySelector(".js-ai-input");
    const text = input.value.trim();
    if (!text) return;
    widget.querySelector(".js-ai-messages").insertAdjacentHTML(
      "beforeend",
      `<p class="workspace-widget__message workspace-widget__message--user">${escapeHtml(text)}</p><p class="workspace-widget__message">I parsed that as a CRM command. JavaScript actions can connect this to tasks, clients, or notes next.</p>`,
    );
    input.value = "";
  });

  const chatTray = document.createElement("section");
  chatTray.className = "communication-tray";
  chatTray.innerHTML = `
    <details>
      <summary>
        <img src="./assets/icons/chat.svg" data-theme-src-dark="./assets/icons/chat.svg" data-theme-src-light="./assets/icons/chat-light-theme.svg" alt="" />
      ...
      <div class="communication-tray__body">
        <a class="btn btn--ghost btn--full" href="tel:+995555010101">Call Alpha Group</a>
        <a class="btn btn--ghost btn--full" href="sms:+995555010101">Text SMS</a>
        <a class="btn btn--ghost btn--full" href="https://wa.me/995555010101" target="_blank" rel="noreferrer">WhatsApp</a>
        <div class="internal-chat">
          <strong>Internal Team Chat</strong>
          <div class="internal-chat__messages js-internal-chat-messages">
            <p><b>Mariam:</b> Please review todayâ€™s hot leads.</p>
          </div>
          <form class="internal-chat__form js-internal-chat-form">
            <input class="input js-internal-chat-input" type="text" placeholder="Message team" />
  ...
  `;
  document.body.append(chatTray);
  chatTray.querySelector(".js-internal-chat-form")?.addEventListener("submit", (event) => {
    event.preventDefault();
    const input = chatTray.querySelector(".js-internal-chat-input");
    const text = input.value.trim();
    if (!text) return;
    chatTray.querySelector(".js-internal-chat-messages").insertAdjacentHTML("beforeend", `<p><b>You:</b> ${escapeHtml(text)}</p>`);
    input.value = "";
  });
})();
